...
void SpaceMouseHID::Init()
{
    Serial1.begin(baudRate, configParameters, pinRx, pinTx);
}

void SpaceMouseHID::SendViewpointDataPacket(uint16_t x, uint16_t y, uint16_t z, uint16_t a, uint16_t b, uint16_t c)
{
    Serial1.write('d');
    SendUInt16(x);
    SendUInt16(y);
...
void SpaceMouseHID::SendUInt16(uint16_t data)
{
...
void SpaceMouseHID::SendButtonsUInt32(uint32_t buttons)
{
    uint8_t nibble = 0;
...
void SpaceMouseHID::SendButtonPacket()
{
...
void SpaceMouseHID::SetButtonMask(uint32_t newButtonMask)
{
    buttonMask = newButtonMask;
}

void SpaceMouseHID::SetButton(uint8_t button_index)
{
...
void SpaceMouseHID::ClearButtonMask()
{
    buttonMask = 0;
}

void SpaceMouseHID::ClearButton(uint8_t button_index)
{
    if ((button_index < SPACEMOUSE_MAX_BUTTON) && (button_index > 0)) {
        buttonMask &= ~(1 << (button_index - 1));
    }
...
void SpaceMouseHID::SendKeyPacket(uint8_t button_index)
{
    Serial1.write('k');
    uint8_t nibble = 0;
...
// TODO: Rework to allow multiple buttons to be pressed at once
void SpaceMouseHID::SendKeyPacketExtended(uint8_t button_index)
{
    Serial1.write('k');
    uint8_t nibble = 0;
    char encoded_zero = EncodeNibble(0);
...
char SpaceMouseHID::EncodeNibble(uint8_t nibble)
{
